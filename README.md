...
- バーコードスキャンは、Android版Chromeなど`BarcodeDetector` API対応ブラウザではネイティブAPIを使用します。iPhone(Safari)など非対応ブラウザでは、自動的に`@ericblade/quagga2`（JS実装のバーコード読み取りライブラリ）に切り替わります。読み取り速度はネイティブAPIよりやや劣り、バーコードとの距離（10〜15cm程度が目安）や明るさに影響されやすいので、うまく読み取れない場合はカメラとの距離を調整してみてください。スキャン画面の下部に現在の状態（起動中／読み取り中／エラー内容）が小さく表示されるので、うまくいかない場合はその文言を確認してみてください。
